    ...
    return (
      <div>
        ...
        <Row gutter={32} type="flex" justify="center" align="middle" style={{ paddingTop: "10%" }}>
          <Col span={16}>
            <Steps>
              <Step status={tokenStatus} title="Authenicating Tokens" icon={<Icon type={tokenStatus === "process" ? "loading" : "key"} />} />

              <Step status={profileStatus} title="Loading Profile" icon={<Icon type={profileStatus === "process" ? "loading" : "solution"} />} />

              <Step status={authStatus} title="Connecting Network" icon={<Icon type={authStatus === "process" ? "loading" : "cloud-download"} />} />
            </Steps>
          </Col>
        </Row>
